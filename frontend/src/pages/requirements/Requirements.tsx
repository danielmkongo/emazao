...
import { Plus, MapPin, Package, Clock, TrendingUp, Filter } from 'lucide-react'
...
export default function Requirements() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<ApiResponse<Requirement[]>>('/requirements?limit=30')
      return res.data
    },
  })
    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      >
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">Post what you need. Receive bids from verified farmers.</p>
        </div>
      ...
      <div className="relative mb-6">
        <input
          ...
          className="w-full h-11 bg-brand-800 border border-white/10 rounded-xl px-4 pl-10 text-white placeholder-white/30 text-sm focus:outline-none focus:border-brand-green transition-all"
        />
      ...
      <div className="flex items-center gap-6 mb-6 text-sm text-white/40">
        <span><span className="text-white font-semibold">{filtered.length}</span> open requirements</span>
        <span className="flex items-center gap-1.5">
          <TrendingUp className="h-4 w-4 text-brand-green" /> Live bidding
        </span>
      </div>
        ...
        <div className="space-y-3">
            ...
            return (
              ...
              >
                <Link to={`/requirements/${req._id}`}>
                  <div className="bg-brand-800 rounded-2xl border border-white/[0.06] p-5 hover:border-brand-green/30 transition-all group">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        {/* Title + badges */}
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <h3 className="font-semibold text-white group-hover:text-brand-lime transition-colors">
                            {req.title}
                          </h3>
                        ...
                        <div className="flex items-center gap-2 mt-3">
                          ...
                          <span className="text-xs text-white/30">{buyer?.name}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        ...
                        <div className="text-right">
                          <p className="text-xs text-white/30">Bids</p>
                          <p className="text-brand-lime font-bold text-xl leading-none">{req.bidCount}</p>
                        </div>
                        {user?.role === 'FARMER' && (
          ...
          {filtered.length === 0 && !isLoading && (
            <div className="text-center py-20">
              ...
              <p className="text-white/40 text-sm mb-6">Be the first to post what you need.</p>
              <Link to="/requirements/post">
