...
export default function AdminVerification() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-bold text-[var(--c-text)] mb-6">Farmer Verification</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-[var(--c-text-3)]">All farmers verified</p>
        </div>
      ) : (
          ...
          {data.map((u, i) => (
            <motion.div key={u._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="flex-1">
                <p className="text-[var(--c-text)] font-medium">{u.name}</p>
                <p className="text-[var(--c-text-3)] text-sm">@{u.username} · {u.location}</p>
                <p className="text-[var(--c-text-4)] text-xs">{timeAgo(u.createdAt)}</p>
              </div>
