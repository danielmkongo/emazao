...
import { Play, Plus, Trash2, Eye, Heart, MessageSquare, Share2, X, Video, Tag, Link as LinkIcon, ExternalLink } from 'lucide-react'
...
export default function DashboardReels() {
  ...
  const [form, setForm] = useState<NewReelForm>(EMPTY_FORM)
  ...
  const canSubmit = form.title.trim() && form.videoUrl.trim()
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-24 border-2 border-dashed border-[var(--c-border)] rounded-2xl">
          ...
          <Button onClick={() => setShowForm(true)}>
            <Plus className="h-4 w-4" /> Upload your first reel
          </Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              ...
              <div className="relative aspect-[9/16] bg-[var(--c-input)] overflow-hidden">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity w-8 h-8 rounded-full bg-black/60 backdrop-blur-sm flex items-center justify-center text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-colors"
                >
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  <p className="text-white font-semibold text-sm leading-tight mb-1.5 line-clamp-1">{reel.title}</p>
              ...
              <div className="px-4 py-3">
                ...
                {reel.caption && (
                  <p className="text-xs text-[var(--c-text-3)] mt-1 line-clamp-2">{reel.caption}</p>
                )}
              </div>
            </motion.div>
          ))}
        ...
        {showForm && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-[var(--c-card)] border border-[var(--c-border)] rounded-2xl shadow-2xl overflow-hidden z-10 max-h-[90vh] overflow-y-auto"
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--c-border)]">
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-full flex items-center justify-center text-[var(--c-text-3)] hover:bg-[var(--c-raised)] transition-colors"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="p-5 space-y-4">
                <Input
                  label="Title *"
                  placeholder="e.g. Harvesting our organic tomatoes"
                  value={form.title}
                ...
                <div>
                  <label className="text-sm font-medium text-[var(--c-text-2)] mb-1.5 flex items-center gap-2">
                    <LinkIcon className="h-3.5 w-3.5 text-brand-green" /> Video URL *
                  </label>
                  <input
                    type="url"
                  ...
                  />
                  <p className="text-xs text-[var(--c-text-4)] mt-1">Upload your video to Cloudinary or any hosting provider and paste the direct URL here</p>
                </div>

                <div>
                  <label className="text-sm font-medium text-[var(--c-text-2)] mb-1.5 flex items-center gap-2">
                    <LinkIcon className="h-3.5 w-3.5 text-[var(--c-text-4)]" /> Thumbnail URL
                  </label>
                  <input
                    type="url"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="tomatoes, organic, harvest (comma-separated)"
                    value={form.tags}
                ...
                {products && products.length > 0 && (
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[var(--c-input)] border border-[var(--c-border)] rounded-xl px-4 py-3 text-[var(--c-text)] text-sm focus:outline-none focus:border-brand-green transition-colors"
                    >
