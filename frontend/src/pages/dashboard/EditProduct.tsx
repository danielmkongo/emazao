...
import { Upload, X, ArrowLeft, Leaf, Trash2 } from 'lucide-react'
...
export default function EditProduct() {
  ...
  const { register, handleSubmit, formState: { errors }, watch, setValue, reset } = useForm<FormData>({
    ...
    defaultValues: { condition: 'FRESH', isOrganic: false, minimumOrder: 1, status: 'ACTIVE' },
  })

  useEffect(() => {
    if (!product) return
    const catId = typeof product.categoryId === 'object' && product.categoryId !== null
      ? (product.categoryId as any)._id ?? product.categoryId
      : product.categoryId
    reset({
      title: product.title,
      ...
      status: (product.status === 'PENDING_REVIEW' ? 'ACTIVE' : product.status) as any,
    })
  ...
  if (!product) return (
    <div className="max-w-2xl mx-auto px-4 py-20 text-center text-[var(--c-text-3)]">Product not found.</div>
  )

  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-[var(--c-text-3)] hover:text-[var(--c-text)] transition-colors">
          <ArrowLeft className="h-4 w-4" /> Back
        </button>
        <Button variant="outline" size="sm" onClick={handleDelete} loading={deleteMutation.isPending}
          className="text-red-400 border-red-400/30 hover:bg-red-400/10">
          <Trash2 className="h-3.5 w-3.5" /> Delete
        </Button>
      </div>

      <h1 className="text-2xl font-bold text-[var(--c-text)] mb-8">Edit Product</h1>

      <form onSubmit={handleSubmit(d => saveMutation.mutate(d as FormData))} className="space-y-6">
        {/* Images */}
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--c-text-2)] mb-1.5">Status</label>
          <select {...register('status')} className="w-full bg-[var(--c-input)] border border-[var(--c-border)] rounded-xl px-4 py-3 text-[var(--c-text)] focus:outline-none focus:border-brand-green">
        ...
        <div>
          ...
          <textarea {...register('description')} rows={3}
            ...
            className="w-full bg-[var(--c-input)] border border-[var(--c-border)] rounded-xl px-4 py-3 text-[var(--c-text)] placeholder:text-[var(--c-text-4)] focus:outline-none focus:border-brand-green resize-none" />
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select {...register('priceUnit')} className="w-full bg-[var(--c-input)] border border-[var(--c-border)] rounded-xl px-4 py-3 text-[var(--c-text)] focus:outline-none focus:border-brand-green">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select {...register('condition')} className="w-full bg-[var(--c-input)] border border-[var(--c-border)] rounded-xl px-4 py-3 text-[var(--c-text)] focus:outline-none focus:border-brand-green">
        ...
        <div>
          ...
          <select {...register('categoryId')} className="w-full bg-[var(--c-input)] border border-[var(--c-border)] rounded-xl px-4 py-3 text-[var(--c-text)] focus:outline-none focus:border-brand-green">
        ...
        {saveMutation.isError && (
          <p className="text-red-400 text-sm">{(saveMutation.error as any)?.response?.data?.message || 'Failed to save product'}</p>
        )}

        <Button type="submit" className="w-full" size="lg" loading={saveMutation.isPending}>
          Save Changes
        </Button>
      </form>
    </div>
