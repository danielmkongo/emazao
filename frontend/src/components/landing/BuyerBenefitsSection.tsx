...
const buyerBenefits = [
  { icon: Search, key: 'one' },
  { icon: ClipboardList, key: 'two' },
  { icon: BadgeCheck, key: 'three' },
] as const
  ...
  <section className="bg-white px-5 py-24 text-[#102014] sm:px-8 lg:px-12 lg:py-28">
    <div className="mx-auto max-w-[1440px]">
      ...
      <figure className="relative mt-10 overflow-hidden rounded-[1.5rem] bg-[#102014] text-white shadow-[0_28px_70px_rgba(16,32,20,.16)] sm:rounded-[2rem] lg:mt-16">
        <div className="relative aspect-[1.15/1] sm:aspect-[16/9] lg:aspect-[2.15/1]">
          ...
          <figcaption className="absolute inset-x-0 bottom-0 flex items-start gap-3 px-5 py-5 sm:hidden">
            ...
            <div><p className="text-[10px] font-medium uppercase tracking-[.18em] text-[#d9ff43]">{t('landing.buyers.orderConfirmed')}</p><p className="mt-1 text-sm text-white/80">{t('landing.buyers.orderConfirmedShort')}</p></div>
          </figcaption>
        </div>
        <figcaption className="hidden items-start gap-4 border-t border-white/10 px-6 py-5 sm:flex sm:items-center sm:px-7">
          ...
          <div className="sm:flex sm:items-baseline sm:gap-3">
            <p className="text-xs font-normal uppercase tracking-[.18em] text-[#d9ff43]">{t('landing.buyers.orderConfirmed')}</p>
            <p className="mt-1 text-sm text-white/70 sm:mt-0">{t('landing.buyers.orderConfirmedLong')}</p>
          </div>
        </figcaption>
      </figure>
      ...
      <div className="-mx-5 mt-8 grid snap-x snap-mandatory auto-cols-[84%] grid-flow-col gap-3 overflow-x-auto px-5 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:-mx-8 sm:auto-cols-[48%] sm:px-8 lg:mx-0 lg:grid-flow-row lg:grid-cols-3 lg:gap-0 lg:overflow-visible lg:border-y lg:border-black/10 lg:px-0 lg:pb-0">
        {buyerBenefits.map(({ icon: Icon, key }, index) => (
          <div key={key} className={`snap-start rounded-2xl bg-[#f2f0e6] p-6 lg:rounded-none lg:bg-transparent lg:px-7 lg:py-8 ${index > 0 ? 'lg:border-l lg:border-black/10' : ''}`}>
            <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#102014] text-[#d9ff43]">
              <Icon className="h-5 w-5" />
            </div>
            <h3 className="mt-5 font-bold tracking-tight">{t(`landing.buyers.${key}Title`)}</h3>
            <p className="mt-2 max-w-sm leading-6 text-black/55">{t(`landing.buyers.${key}Text`)}</p>
          </div>
        ))}
      </div>
      <p className="mt-2 text-center text-xs font-medium text-black/40 lg:hidden">{t('landing.buyers.swipeHint')}</p>
    </div>
  </section>
  )
}
